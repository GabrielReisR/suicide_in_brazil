...
author: "Gabriel R. R."
date: "Última atualização: 10/05/2021"
output: 
  html_document:
    toc: true
    toc_float: true
...
### Sexo
Em geral, os homens cometem mais suicídios que as mulheres no Brasil. 
...
**Importante**: a divisão entre homens e mulheres desconsidera questões envolvendo
gênero, principalmente a cisgeneridade e transgeneridade. Especificamente sobre
a base de dados DATASUS, não foi encontrada uma variável que especificasse 
a identidade de gênero das pessoas registradas no sistema. Nesse sentido, há
falta de informação qualificada sobre o suicídio de pessoas trans em uma das
principais bases de dados pública em saúde do país. Em meio a essa dificuldade,
a Associação Nacional de Travestis e Transsexuais (ANTRA) publicou que houve um 
aumento e 34% no número de suicídios de pessoas transgênero no Brasil em 2020,
quando comparado a 2019 
([link da reportagem](https://www.uol.com.br/vivabem/noticias/redacao/2020/09/09/aumento-do-numero-de-suicidios-entre-populacao-trans-preocupa-ativistas.htm)). Tais dados, entretanto, foram baseados nos 
*casos alcançados* pela ANTRA. A falta de uma variável que codifique isso de
forma acessível no DATASUS dificulta o mapeamento do suicídio de pessoas
transgênero - que pode ser considerado um problema de saúde pública.

### Nível de escolaridade
Analisando por escolaridade, o grupo das pessoas que tiveram entre 
4 e 7 anos de ensino formal foi o mais comum.
...
```{r suicides_by_mothers_education, fig.align= 'center',echo=FALSE, message=FALSE, warning=FALSE}
...
ggplotly(suicides_by_mothers_education, tooltip = 'n')
```

### Trabalho e suicídio
Desde 2010 até 2019, apenas 10 profissões/ocupações estão presentes no topo dos
casos de suicídio. Em números absolutos, a ordem de suicídios por profissão é a
seguinte:
...
estudantes. Separando-se apenas as 10 profissões mais relevantes a cada ano, a
ordem é a seguinte:

## Onde o suicídio acontece?
A maioria dos suicídios, em números brutos, ocorre nas regiões Sul e Sudeste.
...
```{r n_suicides_by_states, echo=FALSE, message=FALSE, warning=FALSE, rows.print=27}
